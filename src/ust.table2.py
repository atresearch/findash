...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
        html.Label("Choose your data period"),
        dcc.RadioItems(
            ['None','1m', '1y','5y','all'],'None',
            id = 'radio-items',
            inline=True
            )], style={'width': '20%', 'display': 'inline-block'}),
    
    html.Div(children=[
        html.Label('or choose your Starting Date: '),
...
def update_table(radio_time_inc, selected_date):
    if radio_time_inc == 'None':
        df_table = build_df(selected_date)
        selected_df = df_table.to_dict('records')
        return selected_df    
    if radio_time_inc == '1m':
        init_date_raw = last_date_raw + dateutil.relativedelta.relativedelta(months=-1)
        init_date = init_date_raw.strftime('%Y-%m-%d')
        df_table = build_df(init_date)
        selected_df = df_table.to_dict('records')
        return selected_df 
    if radio_time_inc == '1y':
        init_date_raw = last_date_raw + dateutil.relativedelta.relativedelta(years=-1)
        init_date = init_date_raw.strftime('%Y-%m-%d')
        df_table = build_df(init_date)
        selected_df = df_table.to_dict('records')
        return selected_df 
    if radio_time_inc == '5y':
        init_date_raw = last_date_raw + dateutil.relativedelta.relativedelta(years=-5)
        init_date = init_date_raw.strftime('%Y-%m-%d')
        df_table = build_df(init_date)
        selected_df = df_table.to_dict('records')
        return selected_df 
    if radio_time_inc == 'all':        
