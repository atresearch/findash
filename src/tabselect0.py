...
app.layout = html.Div(
    [
        ...
        html.Div(id="output-graph", className="five columns"),  
    ],
    className="row"
)
...
def cell_clicked(active_cell):
    ...
    print('---------------------')
    print("We're in cell_clicked")
    
    row = active_cell["row_id"]
    print(f"row id: {row}")

    date = dff.at[row, "Date"]
    print("currently selected Date:",date)        
    
    # keep track of recently selected dates
    # chart the most recent (MAX_CHARTS) of them
    if selected_dates.count(date) == 0:
        selected_dates.append(date)
    if len(selected_dates) > MAX_CHARTS:
        removed_date = selected_dates.pop(0)    
